...
class TestEngine {
    ...
    static calculateGamingAddiction(inputs) {
        const { daily_gaming_hours, monthly_gaming_expense, gaming_years, skip_meals_for_gaming, lose_sleep_for_gaming } = inputs;
        
        let score = 0;
        let riskFactors = [];
        
        // 每日遊戲時間評分 (0-30分)
        if (daily_gaming_hours <= 2) {
            score += 5;
            riskFactors.push('遊戲時間適中，很健康');
        } else if (daily_gaming_hours <= 4) {
            score += 15;
            riskFactors.push('遊戲時間稍長，需要注意');
        } else if (daily_gaming_hours <= 6) {
            score += 25;
        ...
        // 每月花費評分 (0-25分)
        if (monthly_gaming_expense <= 500) {
            score += 0;
            riskFactors.push('遊戲花費合理');
        } else if (monthly_gaming_expense <= 2000) {
            score += 10;
            riskFactors.push('遊戲花費偏高');
        } else if (monthly_gaming_expense <= 5000) {
            score += 20;
        ...
        if (gaming_years >= 10) {
            score += 10;
            riskFactors.push('遊戲資歷深厚，容易成癮');
        } else if (gaming_years >= 5) {
            score += 5;
            riskFactors.push('遊戲資歷中等');
        }
        
        // 跳過用餐影響評分 (0-20分)
        const mealSkipScores = { never: 0, rarely: 5, sometimes: 10, often: 15, always: 20 };
        score += mealSkipScores[skip_meals_for_gaming] || 0;
        if (skip_meals_for_gaming === 'always' || skip_meals_for_gaming === 'often') {
            riskFactors.push('經常為遊戲跳過用餐，影響健康');
        }
        
        // 熬夜遊戲影響評分 (0-20分)
        const sleepSkipScores = { never: 0, rarely: 5, sometimes: 10, often: 15, always: 20 };
        score += sleepSkipScores[lose_sleep_for_gaming] || 0;
        if (lose_sleep_for_gaming === 'always' || lose_sleep_for_gaming === 'often') {
            riskFactors.push('經常為遊戲熬夜，嚴重影響健康');
        }
        ...
        // 計算年度預估花費
        const yearlySpending = monthly_gaming_expense * 12;
        ...
        return {
            ...
            details: {
                score: finalScore,
                statistics: {
                    dailyHours: `${daily_gaming_hours} 小時`,
                    monthlySpending: `NT$ ${monthly_gaming_expense.toLocaleString()}`,
                    yearlySpending: `NT$ ${yearlySpending.toLocaleString()}`,
                    gamingYears: `${gaming_years} 年`,
                    lifestyle: {
                        skipMeals: skip_meals_for_gaming,
                        stayUpLate: lose_sleep_for_gaming
                    }
                }
            },
            suggestions: [recommendation, ...riskFactors.slice(0, 3)]
        };
    }
    ...
    static calculateAgingSimulation(inputs) {
        const { current_age, exercise_level, smoking_status, drinking_frequency, sleep_quality, stress_level } = inputs;
        
        let biologicalAge = parseInt(current_age);
        let lifeExpectancy = 80; // 基準壽命
        let factors = [];
        
        // 吸菸影響
        if (smoking_status === 'heavy') {
            biologicalAge += 8;
            lifeExpectancy -= 10;
            factors.push('重度吸菸：生理年齡+8歲，預期壽命-10年');
        } else if (smoking_status === 'light') {
            biologicalAge += 3;
            lifeExpectancy -= 5;
            factors.push('輕度吸菸：生理年齡+3歲，預期壽命-5年');
        } else if (smoking_status === 'former') {
            biologicalAge += 1;
            lifeExpectancy -= 2;
            factors.push('曾經吸菸：生理年齡+1歲，預期壽命-2年');
        } else {
            factors.push('不吸菸：很好的選擇！');
        }
        
        // 飲酒影響
        if (drinking_frequency === 'heavy') {
            biologicalAge += 5;
            lifeExpectancy -= 7;
            factors.push('重度飲酒：生理年齡+5歲，預期壽命-7年');
        } else if (drinking_frequency === 'moderate') {
            biologicalAge += 1;
            lifeExpectancy -= 2;
            factors.push('適度飲酒：生理年齡+1歲，預期壽命-2年');
        } else if (drinking_frequency === 'rarely') {
            factors.push('很少飲酒：輕微正面影響');
        } else {
            lifeExpectancy += 2;
        ...
        // 運動影響
        if (exercise_level === 'intense') {
            biologicalAge -= 5;
            lifeExpectancy += 8;
            factors.push('高強度運動：生理年齡-5歲，預期壽命+8年');
        } else if (exercise_level === 'moderate') {
            biologicalAge -= 3;
            lifeExpectancy += 5;
            factors.push('適度運動：生理年齡-3歲，預期壽命+5年');
        } else if (exercise_level === 'light') {
            biologicalAge -= 1;
            lifeExpectancy += 2;
            factors.push('輕度運動：生理年齡-1歲，預期壽命+2年');
        } else {
            biologicalAge += 3;
            lifeExpectancy -= 5;
        ...
        if (sleep_quality === 'excellent') {
            biologicalAge -= 3;
            lifeExpectancy += 5;
            factors.push('優質睡眠：生理年齡-3歲，預期壽命+5年');
        } else if (sleep_quality === 'good') {
            biologicalAge -= 1;
            lifeExpectancy += 2;
            factors.push('良好睡眠：生理年齡-1歲，預期壽命+2年');
        } else if (sleep_quality === 'fair') {
            factors.push('一般睡眠：無特殊影響');
        } else {
            biologicalAge += 4;
            lifeExpectancy -= 6;
            factors.push('睡眠品質差：生理年齡+4歲，預期壽命-6年');
        }
        ...
        // 確保合理範圍
        biologicalAge = Math.max(biologicalAge, current_age - 10);
        biologicalAge = Math.min(biologicalAge, current_age + 20);
        lifeExpectancy = Math.max(lifeExpectancy, 60);
        lifeExpectancy = Math.min(lifeExpectancy, 100);
        
        const ageDifference = biologicalAge - current_age;
        const remainingYears = Math.max(lifeExpectancy - current_age, 0);
        ...
        return {
            ...
            unit: '歲',
            level: healthStatus,
            description: `你的生理年齡是 ${Math.round(biologicalAge)} 歲，預期壽命 ${Math.round(lifeExpectancy)} 歲`,
            advice,
            factors,
            details: {
                actualAge: current_age,
    ...
    static calculateFoodExpenseShocker(inputs) {
        const { breakfast_expense, lunch_expense, dinner_expense, snack_expense, delivery_frequency, age } = inputs;
        
        // 計算外食費用
        const dailyCost = breakfast_expense + lunch_expense + dinner_expense + snack_expense;
        const weeklyCost = dailyCost * 7;
        const monthlyCost = dailyCost * 30;
        const yearlyCost = dailyCost * 365;
        
        // 計算外送額外費用 (每次外送假設額外花費30元)
        const deliveryCost = delivery_frequency * 30 * 52; // 每年外送額外費用
        const totalYearlyCost = yearlyCost + deliveryCost;
        
        // 計算在家煮飯的估算成本 (假設是外食的30%)
        const homeCookingCost = dailyCost * 0.3;
        const potentialSavings = {
            daily: dailyCost - homeCookingCost,
            monthly: monthlyCost - (homeCookingCost * 30),
            yearly: totalYearlyCost - (homeCookingCost * 365)
        };
        
        // 估算月薪 (根據年齡)
        let estimatedMonthlyIncome = 30000; // 基本估算
        if (age >= 25) estimatedMonthlyIncome = 40000;
        if (age >= 30) estimatedMonthlyIncome = 50000;
        if (age >= 35) estimatedMonthlyIncome = 60000;
        
        const incomePercentage = (monthlyCost / estimatedMonthlyIncome) * 100;
        
        // 計算震撼指數 (0-100)
        let shockIndex = 0;
        
        // 基於花費金額
        if (totalYearlyCost >= 200000) shockIndex += 30;
        else if (totalYearlyCost >= 150000) shockIndex += 25;
        else if (totalYearlyCost >= 100000) shockIndex += 20;
        else if (totalYearlyCost >= 50000) shockIndex += 15;
        else shockIndex += 10;
        ...
        // 基於外送頻率
        if (delivery_frequency >= 15) shockIndex += 20;
        else if (delivery_frequency >= 10) shockIndex += 15;
        else if (delivery_frequency >= 5) shockIndex += 10;
        
        // 基於每餐平均花費
        const avgMealCost = dailyCost / 4; // 假設一天4餐
        if (avgMealCost >= 200) shockIndex += 15;
        else if (avgMealCost >= 150) shockIndex += 10;
        else if (avgMealCost >= 100) shockIndex += 5;
        
        shockIndex = Math.min(Math.round(shockIndex), 100);
        
        // 計算可以買到的東西
        const alternatives = this.calculateAlternatives(totalYearlyCost);
